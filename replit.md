...
- **Causa raíz** (varias capas):
  1. **Mismatch arquitectónico**: el `editorial-notes-parser` se diseñó para notas LIBRES de un editor humano. Pero los Lectores Holístico y Beta producen MARKDOWN ESTRUCTURADO con secciones específicas (`## SUGERENCIAS CONCRETAS DE CORRECCIÓN`, `## SI FUERA EL AUTOR, CAMBIARÍA...`). El parser LLM tenía que volver a extraer lo que ya estaba estructurado, perdiendo precisión.
  2. **Refiner exige cita literal**: la regla del refiner dice "si la instrucción cita una frase y NO aparece literal en el texto → DESCARTA". Pero los Lectores Holístico/Beta tienen PROHIBIDO citar literal (regla "NO uses citas literales largas (>15 palabras)"). El refiner no encontraba anclaje y descartaba todo.
  3. Resultado: 7 sugerencias perfectas → 0 instrucciones aplicables.
- **Fix en `holistic-reviewer.ts` y `beta-reader.ts`**: añadida una sección final OBLIGATORIA en el system prompt — `## INSTRUCCIONES AUTO-APLICABLES (JSON)` — donde el LLM debe repetir sus sugerencias en formato JSON estructurado entre los marcadores literales `<!-- INSTRUCCIONES_AUTOAPLICABLES_INICIO -->` y `<!-- INSTRUCCIONES_AUTOAPLICABLES_FIN -->`. Schema = `EditorialInstruction[]` (capitulos_afectados, categoria, descripcion, instrucciones_correccion, tipo, prioridad). El LLM YA tiene todo el material; solo tiene que estructurar lo que acaba de escribir, no inventar.
- **Fix en `parseEditorialNotesOnly` (orchestrator)**: detector `extractAutoInstructionsFromReview()` que busca el bloque entre marcadores, extrae el JSON con `repairJson`, valida cada instrucción contra el índice real de capítulos (descarta caps inexistentes), normaliza `tipo`/`prioridad`, y devuelve la lista. Si existe → se usa directamente como `draftInstructions`, saltándose el parser LLM y ahorrando tokens. Si no hay marcadores → flujo humano normal sin cambios.
- **Fix en `groundEditorialInstructions`**: nuevo parámetro `isSystemReview: boolean`. Cuando es `true` y el refiner descarta TODAS las instrucciones, fallback explícito a los borradores con un activity log que explica por qué (los caps ya están verificados, vienen del propio sistema, no fueron inventados por un humano externo). El usuario puede deseleccionar en la previsualización.
- **Resultado esperado**: el informe de "La Sal del Vino Amargo" con 7 sugerencias estructurales ahora produce 7 instrucciones auto-aplicables que aparecen en la previsualización editorial (Eliminar Cap 10, Fusionar Caps 34-35-Epílogo, Reescribir clímax, etc.). El usuario puede aprobar/deseleccionar y aplicar el flujo de corrección.
...
- **Fix**: subido `architect.ts:timeoutMs` de 8 → 12 minutos. Sigue cabiendo dentro del watchdog de 15 min con 3 min de holgura. Mensajes de activity_log actualizados para reflejar "Timeout: 12 min".
- **Por qué no es Fix 9 quien lo causa**: el bloque `betaReaderFeedback` solo se inyecta en la 2ª pasada del Arquitecto (cuando el Lector Beta puntúa < 8). En la pasada inicial el bloque está vacío y el prompt es idéntico al pre-Fix 9. Se diagnosticó tras los logs del usuario en Ubuntu que mostraban Fase 2 fallando antes de que el Lector Beta interviniera.
