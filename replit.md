...
- **[Fix34] Holístico+Beta del reedit con human-in-the-loop (v7.6)**: Replica del "mismo proceso" del pipeline principal en el reedit. (a) Nuevo campo `reeditProjects.pendingEditorialParse: jsonb` (`shared/schema.ts` L640) — requiere `npx drizzle-kit push:pg`. (b) Util reutilizable `server/utils/reedit-editorial-parser.ts` con `parseHolisticBetaForReedit({notesText, chapterIndex, projectTitle})`: primero intenta extraer bloques `<!-- INSTRUCCIONES_AUTOAPLICABLES_INICIO/FIN -->` (camino determinista, sin LLM); si no hay, cae a `EditorialNotesParser.execute()`. Normaliza tipos a `puntual|estructural|global_rename` (auto-aplicables) vs `eliminar|fusionar|global_style|regenerate_chapter|restructure_arc` (administrativos, marcados con `reasonNotAutoApplicable`). Asigna `id` numérico estable para tracking en UI. (c) Stage 8 del reedit ([Fix24]) extendido: tras persistir los `reeditAuditReports` de Holístico+Beta, concatena ambos `notesText` con cabeceras separadoras, llama al parser y persiste `pendingEditorialParse` en el reedit_project. Best-effort: si falla, los informes en bruto siguen disponibles. (d) Nuevo método `ReeditOrchestrator.applyHolisticBetaInstructions(projectId, selectedIds[])`: itera instrucciones seleccionadas, enruta `puntual`/`estructural` a `SurgicalPatcherAgent` por capítulo afectado (con `worldBibleContext` reconstruido desde `reeditWorldBibles`), `global_rename` a find/replace word-boundary determinista vía regex Unicode, y administrativas a skip con razón. Persiste audit_report `holistic_beta_applied` con `{applied, failed, skipped, perInstruction[], summary}` y limpia `pendingEditorialParse`. (e) Tres endpoints nuevos en `server/routes.ts`: `GET .../pending-editorial-parse` (read), `POST .../apply-holistic-beta-instructions` (202 + background con `activeReeditOrchestrators` lock), `DELETE .../pending-editorial-parse` (descarte). (f) UI en pestaña Auditorías de `client/src/pages/reedit.tsx`: nueva `Card` arriba del listado de audit reports, visible solo si hay `pendingEditorialParse`. Cada instrucción con checkbox (auto-seleccionados los `autoApplicable`, deshabilitados los administrativos), badges de tipo/categoría/prioridad, capítulos afectados, razón de no-auto-aplicable cuando aplica. Botones "Aplicar seleccionadas" y "Descartar". `data-testid`: `card-pending-editorial-parse`, `checkbox-instruction-{id}`, `text-instruction-{id}`, `button-apply-holistic-beta`, `button-dismiss-pending-editorial`. Iteración única (no loop): tras aplicar, queda como auditoría descargable; un re-run requeriría rehacer Stage 8 manualmente.
