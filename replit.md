...
- **[Fix139] Auto-revert por REGRESIÓN DE LECTORES: si las correcciones bajan las puntuaciones del Holístico/Beta, se restaura la versión anterior**: el usuario pidió que "cuando se reescribe una novela con indicaciones de los lectores y esta cae en puntuaciones se debería restaurar la versión anterior". **Causa**: tras `applyEditorialNotes`, la re-evaluación de lectores en `recalculateFinalScoreAfterEdits` (`server/orchestrator.ts`) solo AVISABA si bajaban; el único auto-revert (Fix69-C) depende de una caída catastrófica (≥2.0) de la nota del REVISOR FINAL, pero las notas editoriales pueden mantener/subir esa nota y AUN ASÍ empeorar la lectura real (Holístico/Beta). **Arreglo**: nuevo auto-revert por regresión de lectores con guardas conservadoras — `readersRegressed` (ambos lectores con nota nueva y previa, `bestDelta<=0` y `worstDelta<=-1.0`) && `finalNotImproved` (AMBAS notas del Revisor Final presentes y `newScoreRaw<=previousFinalScore`; si mejora o no es comparable, NO se revierte) && hay `modifiedChapterIds` con `preEditContent`. Restaura cada capítulo a `preEditContent` (patrón Fix69-C, `continuityState:null`), repone scores e informes PREVIOS (`finalScore`/`finalReviewResult`, `holisticScore`/`betaScore`, `lastHolisticNotes`/`lastBetaNotes` + timestamps; filosofía Fix134) y hace `return`. **Architect PASS** tras 1er FAIL minor-blocking (exigir ambas notas del Revisor Final para comparar; desacoplar restauración de `finalReviewResult` de `previousFinalScore`; reponer notas por `!== null`). No choca con Fix69-C (cortocircuita antes) ni con el bucle dual Fix81. Limitación: el World Bible modificado NO se revierte (solo la prosa) — se avisa por log. Coste 0 LLM. Sin migración. tsc PASS.

- **[Fix138] La reanudación deja de auto-bloquearse en bucle: un proyecto con el worker muerto vuelve a recuperarse**: el usuario reportó un proyecto "trabado" en su Ubuntu, con el log `[Fix103] Reanudación ignorada: hay actividad muy reciente del proyecto (hace 58s)` repitiéndose. **Causa raíz (dos caras)**: (1) el guard Fix103 (`server/orchestrator.ts`, `_resumeNovel`) aborta la reanudación si el último log del proyecto tiene <120s, pero AL ABORTAR escribe él mismo un log `Reanudación ignorada` — que en el siguiente intento es el más reciente (<120s) y vuelve a bloquear: con el worker REAL muerto, cada clic de "Reanudar" reseteaba la ventana → deadlock. (2) El monitor de congelados y el heartbeat (`server/queue-manager.ts`) medían frescura con `getLastActivityLogTime()`, que incluía ese ruido `Reanudación ignorada`/`Auto-recovery`, manteniendo el reloj de 22 min (`HEARTBEAT_TIMEOUT_MS`) siempre fresco → la auto-recuperación tampoco disparaba. **Arreglo**: nuevo `storage.getLastMeaningfulActivityLogTime(projectId)` (query `WHERE projectId = …`, sin mezclar logs globales `projectId IS NULL`, y `NOT ILIKE` para excluir `Reanudación ignorada`/`Auto-recovery`); lo usan el heartbeat, el monitor de congelados y el guard Fix103. El guard antes leía `getActivityLogsByProject(...,20)` + filtro en memoria, que mezclaba globales y podía dar falso-negativo por truncamiento (permitiendo reanudación CONCURRENTE); ahora consulta por-proyecto. **Architect PASS** (tras 1er FAIL minor-blocking que pedía cerrar justo ese falso-negativo). Intención de Fix103 intacta (un orquestador concurrente real, que emite logs de agentes, sigue bloqueando); caps de auto-recovery intactos. Coste 0 LLM. Sin migración. tsc PASS.
