...
- **[Fix271] Móvil: botones visibles en Series y Catálogo de Libros**: mismo patrón del Fix270b aplicado a las páginas restantes. **`client/src/pages/series.tsx`**: fila de volumen `flex-col sm:flex-row` (badges/botones apilados debajo del título en móvil) + `flex-wrap` en el grupo de acciones. **`client/src/pages/book-catalog.tsx`**: `gap-2` entre título y botones editar/borrar en la cabecera de la tarjeta (el título ya truncaba). Sin cambios en escritorio. Footer `v10.1.6`→`v10.1.7`. Sin migración. tsc PASS.
- **[Fix273] Adjudicador automático de fichas DUDOSAS del fact-check**: el usuario señaló que "¿error o decisión deliberada?" no es pregunta para él — la trama la diseñó el sistema y él nunca leyó la novela. Nuevo agente **`server/agents/fact-dudosa-adjudicator.ts`** (DeepSeek, thinking, maxOutputTokens 16384 — lección deepseek-thinking-output-budget) que recibe el canon (world bible sin reglas `__*`, plan de trama, decisiones, timeline; JSON ≤60k chars) y adjudica cada dudosa: "deliberado" → descartar (prosa intacta; ante la duda, deliberado); "error" con corrección → aplicar vía endpoint apply Fix255 (lotes ≤20). Se ejecuta al final de `runPass` en **`server/services/novel-fact-check.ts`** (tras el auto-apply Fix272), lotes de 15, una sola pasada sin re-verificación (no treadmill). Fichas sin veredicto quedan pendientes (fallback: botones Fix266). Post-review: `state.status="completed"` movido al FINAL de `runPass` (el orquestador espera por status; antes el Holístico podía leer texto a medio corregir durante Fix272/273) + dedupe de decisiones por id (first-write-wins). Footer `v10.1.8`→`v10.1.9`. tsc PASS + smoke PASS.
- **[Fix272] Fact-check: auto-aplicar las corregibles al final de la pasada global**: incoherencia reportada por el usuario — fichas marcadas "corregibles" (incorrecto + sugerencia) quedaban esperando el botón manual. Causa: el bucle por-capítulo (Fix259) rompe en MAX_APPLY_ROUNDS tras la ÚLTIMA verificación sin apply final, así que las fichas re-detectadas en esa ronda nunca recibían intento de corrección. Ahora, al completar la pasada, se ejecuta EN LÍNEA la misma corrección masiva del botón (`runFactCheckPendingApply`) con guard `bulkApplyInFlight`, y el informe final se recalcula con lo realmente pendiente. Sin re-verificación tras aplicar (converge, no treadmill). Las dudosas siguen siendo decisión humana. **`server/services/novel-fact-check.ts`** (final de `runPass`). Footer `v10.1.7`→`v10.1.8`. tsc PASS + smoke PASS.
- **[Fix271] Bucle Beta de Escaletas: refinamiento acumulativo, no lotería**: raíz del "nunca pasa de 7". (1) **Cirugía dirigida**: si TODOS los problemas mayores del Beta citan capítulos concretos, el Cirujano de Escaletas (Fix267) repara SOLO esos caps sobre la escaleta vigente en vez de regenerar World Bible + escaleta desde cero (que arreglaba la queja citada pero re-tiraba los dados sobre el resto: 3 iteraciones, 3 sietes con problemas distintos). Regeneración completa queda como fallback para quejas globales o cirugía fallida. (2) **Guarda determinista de resolución inflada** (`detectBloatedResolution`): >3 caps tras el clímax (último cap con tension_objetivo máxima, solo caps numero>=1) → problema MAYOR citable inyectado al feedback si el Beta pidió revisión; advisory si aprobó. (3) Regla 14b en el prompt Fase 2 del Arquitecto: máximo 3 caps de resolución tras el clímax. **`server/orchestrator.ts`** (bucle beta ~4747, helper tras persistStructuralResiduals), **`server/agents/architect.ts`**. Footer `v10.1.6`→`v10.1.7`. Sin migración. tsc PASS + smoke PASS.
