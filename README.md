# LitAgents v7.5 — Sistema de Orquestacion de Agentes Literarios IA
...
## Novedades v7.3 — Aprovechar el contexto de 1M en reedicion y dos lectores en paralelo

- **[Fix32] Auditor de Cierre de Tramas y Subtramas en post-reedit**: Nuevo agente `PlotThreadClosureAuditorAgent` (`server/agents/plot-thread-closure-auditor.ts`) que lee el manuscrito reeditado COMPLETO de una sola pasada (1M ctx de DeepSeek V4) y produce un INVENTARIO EXHAUSTIVO de todas las tramas (principal, secundarias, subtramas y arcos de personaje) con clasificacion individual de cierre: `cerrada`, `cierre_parcial`, `abierta_intencional` (con justificacion textual obligatoria) o `abierta_colgante` (con `fix_sugerido` accionable). Para cada trama exige evidencia textual breve de apertura y cierre, y devuelve agregados (`puntuacion_cierre 0-10`, `total_*` por estado, `tramas_colgantes_criticas` como resumen ejecutivo). Integrado en el Stage 8 ([Fix24]) como cuarta tarea paralela best-effort y persistido como audit report `auditType: "plot_threads_closure"`. **Detecta automaticamente si el proyecto es VOLUMEN INTERMEDIO de una saga** (consultando otros `reedit_projects` con `seriesId` igual y `seriesOrder` mayor): en ese caso es tolerante con tramas abiertas como ganchos legitimos; si es novela autonoma o ultimo volumen, es estricto y marca como colgantes todas las tramas no cerradas. La instruccion al modelo es "exhaustivo en deteccion, riguroso en clasificacion, prefiere falsos positivos a omisiones": si dudas si algo es trama, INCLUYELO.
- **[Fix24] Stage 8 post-reedit con Holistico + Beta + (opcional) Snapshot de saga**: Tras marcar el reedit como `completed`, se lanza un nuevo `runStage8PostReeditReviews` *best-effort* que ejecuta en paralelo `HolisticReviewerAgent` (editor profesional) y `BetaReaderAgent` (lector cualificado) sobre el manuscrito reeditado completo. Si el proyecto pertenece a una serie, ademas dispara `ManuscriptAnalyzerAgent` para regenerar el snapshot de continuidad sobre la version reeditada. Los tres resultados se persisten como `reeditAuditReports` con `auditType` `holistic_review`, `beta_review` y `series_snapshot` (sin migracion de schema, usan el campo `findings` JSONB existente). Un fallo en este stage NO revierte el reedit ya completado.
- **[Fix25] Beta-Reader paralelo en `runAutoHolisticReviewLoop`**: La auto-revision posterior a finalizar una novela ahora dispara `runHolisticReview` y `runBetaReview` en `Promise.allSettled` (mismo manuscrito, dos perspectivas independientes). Las `notesText` de ambos se concatenan con cabeceras claras ("INFORME DEL LECTOR HOLISTICO" / "INFORME DEL LECTOR BETA") y una nota final indicando al parser que deduplique cuando ambos lectores levantan el mismo problema. Si uno de los dos falla se loguea como warning y se continua con el otro; si ambos fallan, se cierra sin notas.
- **[Fix26] `previousChaptersFullText` + `previousVolumesFullText` en `ChapterExpander` y `NewChapterGenerator`**: Las interfaces `ChapterExpansionInput` y `NewChapterInput` aceptan dos campos opcionales nuevos. Los call sites del reedit (`reedit-orchestrator.ts`) los rellenan con `buildPreviousReeditChaptersFullText` (capitulos previos del propio manuscrito) y `getPreviousVolumesFullTextForReeditById` (volumenes anteriores de la saga). Los prompts insertan ambos como bloques delimitados antes del capitulo a expandir / generar para que el LLM no contradiga ni repita lo ya escrito.
- **[Fix27] Capitulos previos integros en QA del reedit (Voice/Rhythm + Continuity Sentinel)**: Los wrappers `auditContinuity` y `auditVoiceRhythm` del reedit-orchestrator aceptan un nuevo parametro `previousChaptersFullText` que se forwardea al agente canonico. Antes de cada bloque de QA se construye el texto integro de los capitulos PREVIOS al tramo y se pasa al agente. Esto reduce la tasa de falsos positivos por desconocimiento del contexto (ej: una "deriva de voz" en un tramo intermedio que en realidad es coherente con el patron establecido antes). Tambien expuesto en `detectRepetitions` para futuras llamadas parciales.
- **[Fix28] World Bible Extractor con manuscrito completo (presupuesto adaptativo)**: `manuscript-analyzer.ts` ya no recorta cada capitulo a 2000 chars (head 1k + tail 1k) — politica que destruia toda la informacion del medio. Ahora reparte un presupuesto total de **800 000 chars (~200K tokens)** y, si el manuscrito completo cabe, se envia INTEGRO. Solo si excede el presupuesto se recurre al recorte head+tail por capitulo, repartido proporcionalmente y con minimo 2000 chars garantizados por capitulo. Loguea explicitamente cual de las dos rutas tomo.
- **[Fix31] Regen del snapshot de continuidad de la saga al final del reedit**: Integrado dentro del Stage 8 ([Fix24]). Si `reedit_project.seriesId` esta presente, `ManuscriptAnalyzerAgent` (ya con [Fix28]) procesa el manuscrito reeditado completo y persiste el snapshot como audit report `series_snapshot`. Asi los volumenes posteriores ven el canon resultante del reedit, no la version anterior.
