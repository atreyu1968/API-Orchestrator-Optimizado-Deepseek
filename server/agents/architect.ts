...
17. [Fix142] CONTINUIDAD DE SECUNDARIOS CON ARCO. Para cada personaje del world_bible (que NO sea protagonista, antagonista ni traidor/falso aliado) con "arco_transformacion" declarado (≥2 de sus campos estado_inicial/catalizador_cambio/punto_crisis/estado_final con texto real), comprueba que su NOMBRE aparezca de forma REPARTIDA en la escaleta: ≥1 escena de presentación en el acto 1, escenas intermedias sin brechas mayores a ~40% del total de caps, y ≥1 escena en el último tercio (cap ≥ 75% del total) donde RECIBA la consecuencia de su arco (su "estado_final"). PROHIBIDO el cierre fantasma: un secundario que se evapora a mitad del libro y reaparece solo al final para resolver su arco de golpe. Si detectas un secundario abandonado, reparte 1-2 escenas intermedias donde tome una decisión o se posicione, y otra en el último tercio. Si el personaje NO es relevante, QUÍTALE el "arco_transformacion" en el world_bible (un secundario sin arco no exige continuidad).
18. [Fix142] SET-PIECES NO CLONADOS (variedad por CONTENIDO, no solo por etiqueta). Localiza los capítulos que comparten el MISMO tipo de set-piece (p. ej. dos persecuciones, dos asaltos, dos interrogatorios, dos huidas). Aunque lleven "forma_dominante" distinta, si el LECTOR percibiría la misma coreografía (corre→se esconde→escapa repetido), cada repetición DEBE variar al menos en: escenario (lugar y sus reglas), táctica del protagonista (qué hace distinto), oposición (quién/qué le presiona) y COSTE pagado (qué pierde aquí que no perdió antes). Refleja esa variación en "objetivo_narrativo"/"eventos_pivotales"/"beats" del cap, no solo en la etiqueta. Dos set-pieces del mismo tipo con la misma coreografía y el mismo coste = repetición percibida; reescribe uno. NOTA: esta regla la verifica ahora un AUDITOR DETERMINISTA (dimensión "set_piece_clonado"): agrupa los caps por tipo de set-piece según "funcion_estructural" y marca un par cuando comparten táctica ("objetivo_narrativo"/"beats") MÁS ≥2 de {escenario ("ubicacion"), oposición ("elenco_presente"), coste sin escalar ("apuesta_dramatica")}. Para pasar el gate, asegúrate de que cada set-piece repetido difiera en esos ejes y declara esos campos con contenido REAL y específico (no genérico).
...
${input.seriesUnifiedWorldBible}
    ...
    - [Fix192] Los NOMBRES PROPIOS de estos hitos/hilos son el CANON de la
      serie: úsalos LITERALMENTE en escaleta y world_bible. PROHIBIDO
      renombrar personajes, lugares u objetos citados aquí (causa real de
      rechazo editorial: la guía decía un nombre y el libro usó otro).
