...
export class BetaReaderAgent extends BaseAgent {
  ...
  async runReview(
    ...
    const previousNotesBlock = (input.previousBetaNotes && input.previousBetaNotes.trim().length > 200)
      ? `\n\n═══════════════════════════════════════════════════════════════════\n## NOTAS DE TU LECTURA ANTERIOR (referencia de evolución)\n═══════════════════════════════════════════════════════════════════\n\n${input.previousBetaNotes.slice(0, 24000)}\n\nIMPORTANTE: arriba están las impresiones que TÚ MISMO emitiste la última vez. ESTA RELECTURA NO TE EXIME DE EMITIR INSTRUCCIONES — al contrario:\n- Si una observación previa SIGUE vigente porque el autor no la corrigió, REPITELA en el JSON con MÁS urgencia y prioridad subida (de "media" a "alta"). Reformula la instrucción para que sea más concreta y accionable que la vez anterior. No basta con decir "ya lo dije" — es tu trabajo insistir.\n- Si una observación previa YA ESTÁ resuelta, dilo brevemente en prosa ("la pega del cap 12 ya no me molestó") y libera ese hueco para una MEJORA INCREMENTAL nueva: algo que antes no captaste, un retoque para subir el nivel hacia 9+, una oportunidad de pulido fino.\n- El umbral comercial deseable es Beta >= 9. Si vienes de un 7 o un 8, tu trabajo en ESTA lectura es identificar exactamente qué cambios cerrarían ese gap. Si vienes ya de un 9, identifica qué llevaría la obra al 10.\n- PROHIBIDO devolver \`{"instrucciones": []}\` con la excusa de "ya lo dije la otra vez" o "no quiero repetirme". El JSON SIEMPRE lleva 3+ entradas: viejas reforzadas, nuevas detectadas, o mejoras incrementales hacia la siguiente nota.`
      : "";
