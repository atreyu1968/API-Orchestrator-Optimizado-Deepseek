    ...
    try {
      ...
      const registry = allSeries.map(s => {
        ...
        const seriesManuscripts = allManuscripts
          ...
          .sort((a, b) => (a.seriesOrder || 0) - (b.seriesOrder || 0));
        const seriesReeditProjects = allReeditProjects
          .filter(rp => rp.seriesId === s.id)
          .sort((a, b) => (a.seriesOrder || 0) - (b.seriesOrder || 0));
        ...
        const volumes = [
          ...
          ...seriesReeditProjects.map(rp => ({
            type: "reedit" as const,
            id: rp.id,
            title: rp.title,
            seriesOrder: rp.seriesOrder,
            status: rp.status === "completed" ? "completed" : rp.status,
        ...
        return {
          ...
          reeditProjects: seriesReeditProjects,
          volumes,
...
Escribe en formato Markdown claro y organizado. Sé específico con datos concretos de la serie original — no inventes datos que no aparezcan en el contexto proporcionado.`;
  ...
  app.post("/api/reedit-projects/convert-to-series", async (req: Request, res: Response) => {
    try {
      ...
      let pseudonymName: string | undefined;
      if (targetPseudonymId) {
        const pseud = await storage.getPseudonym(targetPseudonymId);
        if (pseud) pseudonymName = pseud.name;
      }
...
LOCALIZACIONES:
${JSON.stringify(allLocations.slice(0, 50), null, 2)}

LÍNEA TEMPORAL:
${JSON.stringify(allTimeline.slice(0, 50), null, 2)}

REGLAS DEL MUNDO:
${JSON.stringify(allLoreRules.slice(0, 30), null, 2)}

Genera un JSON con esta estructura exacta:
{
  "characters": [{"name": "...", "description": "...", "firstAppearance": "Libro X, Cap Y", "aliases": [], "relationships": [], "arc": "..."}],
  "locations": [{"name": "...", "description": "...", "firstMention": "Libro X", "characteristics": []}],
  "timeline": [{"event": "...", "chapter": "Libro X, Cap Y", "timeMarker": "...", "importance": "high/medium/low"}],
  "loreRules": [{"rule": "...", "source": "Libro X", "category": "..."}],
  "seriesArcs": [{"arc": "...", "startBook": 1, "currentStatus": "...", "resolution": "..."}]
}
...
`;
  ...
  const assessCooldowns = new Map<number, number>();

  app.post("/api/projects/:id/assess-reedit", async (req: Request, res: Response) => {
    try {
      const projectId = parseInt(req.params.id);
      ...
      const completedChapters = chapters.filter(c => c.content && c.content.trim().length > 0);
      if (completedChapters.length === 0) {
      ...
      const sorted = completedChapters.sort((a, b) => a.chapterNumber - b.chapterNumber);
...
- Género: ${project.genre}
- Tono: ${project.tone}
- Puntuación final previa: ${project.finalScore ? project.finalScore + "/10" : "Sin evaluar"}
- Total capítulos: ${completedChapters.length}
...
- Considera que la re-edición puede subir 1-2 puntos, pero no hacer milagros con prosa muy pobre`;
      ...
      res.json({
        projectId,
        title: project.title,
        existingFinalScore: project.finalScore,
...
- Considera que la re-edición puede subir 1-2 puntos, pero no hacer milagros con prosa muy pobre`;
  ...
  app.post("/api/audiobooks/:id/generate", async (req: Request, res: Response) => {
    ...
    try {
      ...
      async function processChapter(chapter: any) {
        try {
          const result = await generateChapterAudio(chapter);
          ...
          if (ctrl.signal.aborted || chapterError.name === "AbortError" || chapterError.message === "Generation cancelled") {
            ...
            await storage.updateAudiobookChapter(chapter.id, { status: "error", errorMessage: chapterError.message });
            hasError = true;
          }
        }
      }
      ...
      for (let i = 0; i < chapters.length; i += CONCURRENCY) {
        if (abortController.signal.aborted || wasCancelled) {
          wasCancelled = true;
          break;
        }
        const batch = chapters.slice(i, i + CONCURRENCY);
        console.log(`[Audiobook] Project ${projectId}: Processing batch of ${batch.length} chapters (${i + 1}-${Math.min(i + CONCURRENCY, chapters.length)}/${chapters.length})`);
        await Promise.all(batch.map(ch => processChapter(ch)));
      }

      if (abortController.signal.aborted) wasCancelled = true;

      const updatedChapters = await storage.getAudiobookChaptersByProject(projectId);
      completedCount = updatedChapters.filter((ch: any) => ch.status === "completed").length;
